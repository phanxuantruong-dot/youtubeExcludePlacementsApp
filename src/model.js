  ...
  constructor() {
    ...
    this._gdnCtrAndAllConvGAQL = `
      ...
      AND segments.date DURING TODAY
        `;
    this._gdnSitesUrlsGAQL = `
      SELECT
      group_placement_view.target_url,  
      segments.ad_network_type,
      metrics.all_conversions
      FROM group_placement_view
      WHERE campaign.name REGEXP_MATCH "(?i).*truong.*" 
      AND segments.ad_network_type = CONTENT
      AND metrics.all_conversions < 1
      AND segments.date DURING TODAY
          `;
    this._domainMustHave = [`.us`, `.com`, `.net`, `.org`, `.google`];
    this._domainMustNotHave = [
      `wordfeud`,
      `haber`,
      `usercash`,
      `aaa`,
